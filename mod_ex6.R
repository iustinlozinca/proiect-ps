# Module Exercise 6: Probabilități Condiționate și Condiționări
...
ui_ex6 <- function(id) {
  ...
  fluidPage(
    ...
    sidebarLayout(
      ...
      mainPanel(
        tabsetPanel(
          tabPanel("6.a) Probabilități Condiționate",
          ...
          tabPanel("6.b) Speranțe Condiționate",
...
server_ex6 <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    # 6.a) Tabel probabilități condiționate
    output$tabCondProb <- renderTable({
      ...
      p_b_given_a <- mean(df$T[ev_a] <= t0)

      data.frame(
        Probabilitate = c("P(A)", "P(B)", "P(C)", "P(A | N ≤ n0)", "P(B | A)"),
        Valoare = c(mean(ev_a), mean(ev_b), mean(ev_c), p_a_given_c, p_b_given_a),
        Descriere = c("Succes final", "SLA respectat", "Puține încercări",
    ...
    output$plotCondA <- renderPlot({
      ...
      ggplot(success_by_n, aes(x = factor(N), y = I)) +
        geom_bar(stat = "identity", fill = "steelblue") +
        geom_hline(yintercept = mean(df$I[df$N <= n0]), color = "red", linetype = "dashed", linewidth = 1) +
        geom_vline(xintercept = n0 + 0.5, color = "orange", linewidth = 1.2) +
        labs(title = "Rata de Succes per Număr de Încercări",
             x = "N (încercări)", y = "P(A | N)") +
        ylim(0, 1) +
        theme_minimal()
    })

    # Grafic distribuție T | succes
    output$plotCondB <- renderPlot({
      ...
      ggplot(df_s, aes(x = T)) +
        ...
        geom_vline(xintercept = t0, color = "red", linewidth = 1.2, linetype = "dashed") +
        labs(title = "Distribuția T | Succes", x = "Timp (s)", y = "Densitate") +
        theme_minimal()
    })
    ...
    # 6.b) Tabel speranțe condiționate
    output$tabCondExp <- renderTable({
      ...
      data.frame(
        ...
        `E(T)` = c(mean(df_s$T), if (nrow(df_f) > 0) mean(df_f$T) else NA, mean(df$T)),
        Mediana = c(median(df_s$T), if (nrow(df_f) > 0) median(df_f$T) else NA, median(df$T)),
        Var = c(var(df_s$T), if (nrow(df_f) > 1) var(df_f$T) else NA, var(df$T))
      )
    }, digits = 4)

    # Histogramă suprapusă
    output$plotCondExp <- renderPlot({
      df <- sim_data()
      df$Status <- ifelse(df$I == 1, "Succes", "Eșec")

      ggplot(df, aes(x = T, fill = Status)) +
        ...
        labs(title = "Distribuția T Condiționată de I", x = "Timp (s)", y = "Densitate") +
        theme_minimal()
    })
    ...
    output$boxplotCondExp <- renderPlot({
      ...
      ggplot(df, aes(x = Status, y = T, fill = Status)) +
    ...
    output$explainCondExp <- renderText({
      df <- sim_data()
      e_s <- mean(df$T[df$I == 1])
      n_failures <- sum(df$I == 0)
      e_f <- if (n_failures > 0) mean(df$T[df$I == 0]) else NA
      ...
      if (!is.na(e_f)) {
        ...
        paste0(
          ...
          "Explicație: P(eșec total) = (1-", input$p_succes, ")^", input$max_retry,
    ...
    output$interpretareUX <- renderUI({
      ...
      p_b_a <- mean(df$T[df$I == 1] <= t0)
      e_s <- mean(df$T[df$I == 1])
      e_f <- if (sum(df$I == 0) > 0) mean(df$T[df$I == 0]) else NA
      ...
      HTML(paste0(
        ...
        "<p>", if (p_a_c > p_a) {
          "Cererile cu puține încercări au o rată de succes mai mare decât media."
        } else {
        ...
        if (!is.na(e_f)) {
          paste0("<p>E(T | I=0) = <b>", round(e_f, 2), "</b> secunde</p>",
          ...
          paste0("<p>E(T | I=0) = <b>N/A</b></p>",
                 ...
                 "Pentru ", input$n_sim, " simulări, numărul așteptat de eșecuri este ~",
