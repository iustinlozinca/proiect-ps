...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput(
        "p_succes", "Probabilitate succes (per încercare):", 0.1, 0.9, 0.7
      ),
      sliderInput("max_retry", "Nr. Maxim retry:", 1, 10, 3),
      hr(),
      h4("Praguri Evenimente"),
      sliderInput(
        "t0_SLA", "Prag Timp (t0) - SLA:", 0.5, 10.0, 2.0,
        step = 0.5
      ),
...
server <- function(input, output) {
  ...
  output$tabProb <- renderTable({
    ...
    # Cel puțin un eșec (adică nu a reușit din prima sau a eșuat total)
    # D: IF N=1 and I=1 THEN 0 failures. ELSE at least 1 failure.
